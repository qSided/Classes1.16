...
import com.qsided.classesmod.network.ClassDataMessage;
...
public class ClassEvents extends GuiElement {
	...
	public static void playerTickEvent(TickEvent.PlayerTickEvent event) {
		...
		if (entity instanceof ServerPlayerEntity) {
			...
			if (player.getHeldItem(Hand.MAIN_HAND).getItem() instanceof AxeItem
					|| player.getHeldItem(Hand.MAIN_HAND).getItem() instanceof PickaxeItem
							&& ClassesSavedData.get(player.server).getPlayerClass(player).equals("FREEMA")) {
		...
		if (entity instanceof ServerPlayerEntity) {
			...
			if (ClassesSavedData.get(player.server).getPlayerClass(player).equals("SOLDIE")) {
				if (player.getHeldItem(Hand.MAIN_HAND).getItem() instanceof SwordItem
		...
		if (entity instanceof ServerPlayerEntity) {
			...
			if (ClassesSavedData.get(player.server).getPlayerClass(player).equals("MAGICIA")) {
				if (((EnchantmentHelper.getEnchantmentLevel(Enchantments.UNBREAKING,
						((ServerPlayerEntity) entity).getHeldItemMainhand())) <= 1)) {
	...
	@SubscribeEvent
	public static void playerJoinedWorldEvent(final PlayerLoggedInEvent event) {
		ServerPlayerEntity player = (ServerPlayerEntity) event.getPlayer();

		if (!event.getPlayer().world.isRemote()) {
			PacketHandler.sendToClient(new ClassDataMessage(PlayerClassHandler.encodeForClient()), player);
		}
		...
		if (ClassesSavedData.get(player.server).getPlayerClass(player).equals("MINER")) {
			...
			player.sendStatusMessage(new StringTextComponent(
					TextFormatting.GRAY + "You're class has been updated to: " + TextFormatting.GOLD + "Worker"),
					false);
		}
		...
		if (ClassesSavedData.get(player.server).getPlayerClass(player).equals("WARRIOR")) {
			...
			player.sendStatusMessage(new StringTextComponent(
					TextFormatting.GRAY + "You're class has been updated to: " + TextFormatting.RED + "Fighter"),
					false);
		}
		...
		if (ClassesSavedData.get(player.server).getPlayerClass(player).equals("BLACKSMITH")) {
			...
			player.sendStatusMessage(new StringTextComponent(
					TextFormatting.GRAY + "You're class has been updated to: " + TextFormatting.BLUE + "Artisan"),
					false);
		}
		if (ClassesSavedData.get(player.server).getPlayerClass(player).equals("FARMER")) {
			...
			player.sendStatusMessage(new StringTextComponent(
					TextFormatting.GRAY + "You're class has been updated to: " + TextFormatting.GREEN + "Survivor"),
					false);
		}
		...
		if (ClassesSavedData.get(player.server).getPlayerClass(player).equals("FIGHTER")) {
			...
			player.sendStatusMessage(new StringTextComponent(
					TextFormatting.GRAY + "You're class has been updated to: " + TextFormatting.RED + "Soldier"),
					false);
		}
		if (ClassesSavedData.get(player.server).getPlayerClass(player).equals("WORKER")) {
			...
			player.sendStatusMessage(new StringTextComponent(
					TextFormatting.GRAY + "You're class has been updated to: " + TextFormatting.AQUA + "Freeman"),
					false);
		}
		if (ClassesSavedData.get(player.server).getPlayerClass(player).equals("SURVIVOR")) {
			...
			player.sendStatusMessage(new StringTextComponent(
					TextFormatting.GRAY + "You're class has been updated to: " + TextFormatting.GOLD + "Survivalist"),
					false);
		}
		if (ClassesSavedData.get(player.server).getPlayerClass(player).equals("EXPLORER")) {
			...
			player.sendStatusMessage(new StringTextComponent(
					TextFormatting.GRAY + "You're class has been updated to: " + TextFormatting.GREEN + "Adventurer"),
					false);
		}
		if (ClassesSavedData.get(player.server).getPlayerClass(player).equals("MAGICIAN")) {
			...
			player.sendStatusMessage(
					new StringTextComponent(
							TextFormatting.GRAY + "You're class has been updated to: " + TextFormatting.BLUE + "Mage"),
					false);
			...
			if (ClassesConfigs.COMMON.doDailyRewards.get()) {
				...
				if (currentTime - ClassesSavedData.get(player.server).getPlayerClaimed(player) >= 720L) {
					...
					if (ClassesSavedData.get(player.server).getPlayerClass(player).equals("SOLDIER")) {
						...
						player.sendStatusMessage(new StringTextComponent(
								TextFormatting.DARK_GRAY + "You claimed your daily reward of " + TextFormatting.RED
										+ "1 Golden Apple" + TextFormatting.DARK_GRAY + " by logging in today!"),
								false);
						ClassesSavedData.get(player.server).setPlayerClaimed(player,
					...
					if (ClassesSavedData.get(player.server).getPlayerClass(player).equals("SURVIVALIST")) {
						...
						player.sendStatusMessage(new StringTextComponent(
								TextFormatting.DARK_GRAY + "You claimed your daily reward of " + TextFormatting.GREEN
										+ "6 Hay Bales" + TextFormatting.DARK_GRAY + " by logging in today!"),
								false);
						ClassesSavedData.get(player.server).setPlayerClaimed(player,
								System.currentTimeMillis() / 1000 / 60);
					}
					...
					if (ClassesSavedData.get(player.server).getPlayerClass(player).equals("ARTISAN")) {
						...
						player.sendStatusMessage(new StringTextComponent(
								TextFormatting.DARK_GRAY + "You claimed your daily reward of " + TextFormatting.YELLOW
										+ "1 Anvil" + TextFormatting.DARK_GRAY + " by logging in today!"),
								false);
						ClassesSavedData.get(player.server).setPlayerClaimed(player,
								System.currentTimeMillis() / 1000 / 60);
					}
					...
					if (ClassesSavedData.get(player.server).getPlayerClass(player).equals("FREEMAN")) {
						...
						player.sendStatusMessage(new StringTextComponent(
								TextFormatting.DARK_GRAY + "You claimed your daily reward of " + TextFormatting.GOLD
										+ "12 Iron Ore" + TextFormatting.DARK_GRAY + " by logging in today!"),
								false);
						ClassesSavedData.get(player.server).setPlayerClaimed(player,
								System.currentTimeMillis() / 1000 / 60);
					}
					...
					if (ClassesSavedData.get(player.server).getPlayerClass(player).equals("ADVENTURER")) {
						...
						ItemHandlerHelper.giveItemToPlayer((player), giveHayBales);
						player.sendStatusMessage(new StringTextComponent(
								TextFormatting.DARK_GRAY + "You claimed your daily reward of " + TextFormatting.AQUA
										+ "1 Diamond" + TextFormatting.DARK_GRAY + " by logging in today!"),
								false);
						ClassesSavedData.get(player.server).setPlayerClaimed(player,
								System.currentTimeMillis() / 1000 / 60);
					}
					...
					if (ClassesSavedData.get(player.server).getPlayerClass(player).equals("MAGE")) {
						...
						player.sendStatusMessage(new StringTextComponent(TextFormatting.DARK_GRAY
								+ "You claimed your daily reward of " + TextFormatting.DARK_BLUE + "8 Xp Bottles"
								+ TextFormatting.DARK_GRAY + " by logging in today!"), false);
						ClassesSavedData.get(player.server).setPlayerClaimed(player,
								System.currentTimeMillis() / 1000 / 60);
					}
				} else {
					player.sendStatusMessage(
							new StringTextComponent(
									TextFormatting.DARK_RED + "You have already claimed your daily login reward!"),
							false);
